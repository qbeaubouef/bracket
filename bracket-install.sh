...
GITHUB_RAW="https://raw.githubusercontent.com/qbeaubouef/bracket/main"
pct exec "$CT_ID" -- bash -c "
  curl -fsSL ${GITHUB_RAW}/server.js -o /opt/bracket/server.js 2>/dev/null
  curl -fsSL ${GITHUB_RAW}/index.html -o /opt/bracket/public/index.html 2>/dev/null
  if [ ! -s /opt/bracket/public/index.html ]; then
    echo '<html><body style=\"background:#0a0e14;color:#fff;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh\"><div style=\"text-align:center\"><h1>🏀 Beaubouef Bracket</h1><p>Upload index.html and server.js to GitHub repo</p></div></body></html>' > /opt/bracket/public/index.html
  fi
" &>/dev/null

# Package.json
pct exec "$CT_ID" -- bash -c 'cat > /opt/bracket/package.json << PKEOF
{"name":"beaubouef-bracket","version":"2.0.0","main":"server.js","dependencies":{"express":"^4.18.2"}}
PKEOF'
