...
# License: MIT | https://github.com/qbeaubouef/baseball

# ── App Defaults ──────────────────────────────────────────────────────────────
APP="MLB Dashboard"
var_tags="baseball;mlb"
var_cpu="1"
var_ram="512"
var_disk="2"
var_os="debian"
...
/_/  /_/_____/_____/
    ____            __    __                         __
   / __ \____ ___  / /_  / /_  ____  ____ __________/ /
  / / / / __ `/ / / / / / __ \/ __ \/ __ `/ ___/ __  /
 / /_/ / /_/ / /_/ / / / /_/ / /_/ / /_/ / /  / /_/ /
/_____/\__,_/\__,_/ /_/_.___/\____/\__,_/_/   \__,_/

EOF
  echo -e "          ${BL}⚾  Self-hosted MLB Dashboard  ⚾${CL}\n"
}
...
# Package.json
pct exec "$CT_ID" -- bash -c 'cat > /opt/baseball/package.json << PKEOF
{"name":"mlb-dashboard","version":"1.0.0","main":"server.js","dependencies":{"express":"^4.18.2"}}
PKEOF'

# npm install
pct exec "$CT_ID" -- bash -c "cd /opt/baseball && npm install --production &>/dev/null" &>/dev/null
msg_ok "Installed ${APP}"

# ── Create Service ────────────────────────────────────────────────────────────
msg_info "Creating service"
pct exec "$CT_ID" -- bash -c 'cat > /etc/systemd/system/baseball.service << SVCEOF
[Unit]
...
systemctl enable -q baseball.service
systemctl start baseball.service'
...
echo -e "${TAB}${GN}PIN:         ${YW}4406${CL}"
echo ""
echo -e "${TAB}Point your reverse proxy / tunnel at ${GN}${LXC_IP}:3000${CL}"
echo -e "${TAB}Edit team colors: ${BL}/opt/baseball/team_colors_corrected_v3.csv${CL}"
echo -e "${TAB}Go Stros. ⚾"
echo ""
